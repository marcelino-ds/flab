...
//
// Desain prompt: model BOLEH menalar singkat dulu, lalu WAJIB menutup dengan satu
// blok JSON. Memaksa "JSON only, no reasoning" terbukti menurunkan akurasi pada
// soal yang butuh langkah (koding, matematika, MCQ multi-langkah). Extractor selalu
// mengambil blok JSON TERAKHIR, jadi reasoning di depan aman.
...
- ISIAN SINGKAT / NUMERIK / CLOZE → "jawaban": jawaban presisi. Jika ADA LEBIH DARI SATU kotak isian, WAJIB array of strings sesuai urutan kotak: ["isi 1", "isi 2"]. Jika 1 kotak, string biasa. "index_pilihan": 0.
...
Selesaikan soal di atas. Kamu boleh berpikir singkat (maksimal 2-3 kalimat) untuk menalar jawaban.
SETELAH ITU, WAJIB akhiri jawabanmu dengan TEPAT SATU blok JSON berikut sebagai baris terakhir — tanpa teks apa pun sesudahnya:
\`\`\`json
${ANSWER_SHAPE}
\`\`\`
${buildAnswerRules()}
